package classes.chart.parse {
	...
	public class ChartStepmania extends NoteChart {
		...
		public function ChartStepmania(id:Number, inData:String, framerate:int = 60):void {
			...
			var notesParsed:Boolean = false;
			var lineNum:int = 0;
			while (chartData[lineNum] != null) {
				var lineText:String = String(chartData[lineNum]);
				
				// Song Meta Data
				if (lineText.substr(0, 7) == "#TITLE:") {	// #TITLE
					this.songInfo.title = lineText.substring(7, lineText.length - 1);
				}
				if (lineText.substr(0, 10) == "#SUBTITLE:") {	// #SUBTITLE
					this.songInfo.subtitle = lineText.substring(10, lineText.length - 1);
				}
				if (lineText.substr(0, 8) == "#ARTIST:") {	// #ARTIST
					this.songInfo.author = lineText.substring(8, lineText.length - 1);
				}
				if (lineText.substr(0, 8) == "#CREDIT:") {	// #CREDIT
					this.songInfo.stepauthor = lineText.substring(8, lineText.length - 1);
				}
				...
				//- Set Notes
				if (lineText.substr(0, 4) == "#NOT" && !notesParsed) {	// #NOTES
					notesParsed = true;
